...
| `tests/` | Tesztkészlet (368 teszt): domain-logika, config, logging, BLE reconnect, async adatsík – `pytest tests/` |
...
A készlet 368 tesztet tartalmaz: tiszta domain-logika (zónák, cooldown, átlagolás), config-validáció, logging, BLE reconnect forgatókönyvek, valamint a teljes async adatsík (power minta → zóna parancs → dropout) és a Zwift protobuf dekóder.
